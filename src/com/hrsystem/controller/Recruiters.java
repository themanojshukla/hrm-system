...
public class Recruiters extends HttpServlet {
    ...
    private static final long serialVersionUID = 1585123087840569393L;
	...
	if (name == null || email == null || passwd == null || skills == null || name.isEmpty() || email.isEmpty()
		|| passwd.isEmpty() || skills.isEmpty()) {
		...
		session.setAttribute("color", "red");
		session.setAttribute("recAddMessage", "Oops..!! This is an existing Recruiter.");
		response.sendRedirect(request.getContextPath() + "/admin/");
	    } else {
		recruiterDao.addNewRecruiter(recruiter);
		session.setAttribute("color", "green");
		session.setAttribute("recAddMessage", "New Recruiter added Successfully.");
		response.sendRedirect(request.getContextPath() + "/admin/");
	    }
	}
    }
